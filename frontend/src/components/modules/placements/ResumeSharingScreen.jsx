      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Select Placement Drive</label>
              <select
                value={selectedDriveId}
                onChange={(e) => setSelectedDriveId(e.target.value)}
                className="font-extrabold text-slate-900 text-sm bg-transparent outline-none cursor-pointer"
              >
                {drives.map(d => (
                  <option key={d._id} value={d._id}>{d.companyName} — {d.jobRole} ({d.packageCTC})</option>
                ))}
              </select>
            </div>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white text-xs font-bold px-5 py-2 rounded-xl transition shadow-sm disabled:opacity-50"
            >
